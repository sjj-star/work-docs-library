...
1. **LLM 是外部 Agent 的资源，不是插件的固定成本**
   - 评估框架、智能体检索、查询改写等需要 LLM 评判或推理的能力，优先以 **Skill 编排 + 原子 MCP 工具** 实现，由外部 Agent 承担 LLM 调用成本，而不是在插件内部启动完整 Agent 运行时。
...
4. **状态安全优先于智能**
   - 即使 Agent 可以调用写工具，数据变更类操作仍应保持在 `admin_tools.py` 或显式 MCP 写工具中，保留人工/审计边界。
5. **可观测性应面向 Agent 理解**
