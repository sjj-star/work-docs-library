...
- `search.include_graph`: default `true` (set explicitly for clarity). Keep it `true` so results include entities and relations that drive the next `explore` step. Only set `false` if you only need raw text chunks.
