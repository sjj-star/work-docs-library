...
| **六阶段 Pipeline 需要阶段级状态机** | 无 LLM API 时无法区分「已跳过」与「已完成」，无法安全恢复 | 新增 `pipeline_stage_status` 表；`_process_one` 按阶段状态恢复；LLM/Embedding 未配置时对应阶段标记 `skipped`；stage4 写入 blocks 但实体为空；后续配置齐全后自动补跑 |
