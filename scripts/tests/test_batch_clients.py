...
def test_submit_embedding_batch_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(Config, "DB_PATH", tmp_path / "workdocs.db")
    monkeypatch.setattr(Config, "EMBEDDING_MODEL", "embedding-3")
    monkeypatch.setattr(Config, "EMBED_ARRAY_MAX_SIZE", 64)

    client = BatchClient(api_key="test-key", base_url="https://test.bigmodel.cn")

    captured_requests = []

    def _fake_submit_parallel_batches(reqs, timeout=None, output_path=None):
        captured_requests.extend(reqs)
        # 故意乱序返回，验证是否按 custom_id + data.index 排序
        return [
            {
                "custom_id": "embed_1",
                "response": {"body": {"data": [{"index": 0, "embedding": [0.1, 0.2]}]}},
            },
            {
                "custom_id": "embed_0",
                "response": {"body": {"data": [{"index": 0, "embedding": [0.3, 0.4]}]}},
            },
        ]

    monkeypatch.setattr(client, "submit_parallel_batches", _fake_submit_parallel_batches)
    ...
    assert captured_requests[0]["body"]["input"] == ["hello", "world"]
