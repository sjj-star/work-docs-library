...
def test_search_with_graph_basic(tmp_path, monkeypatch):
    ...
    # 插入 chunk
    ck = Chunk(doc_id="d1", chunk_id="c1", content="Test content about PIEIER register")
    ...
    assert result["chunks"][0]["chunk"].content == ck.content
...
def test_get_content_with_entities(tmp_path, monkeypatch):
    ...
    # 插入 chunk
    ck = Chunk(doc_id="d1", chunk_id="c1", content="Module A contains Signal B")
    db_id = svc.db.insert_chunk(ck)
    ...
    assert result["chunk"].content == ck.content
...
def test_get_content_with_entities_not_found(tmp_path, monkeypatch):
    ...
    svc = _make_service(tmp_path, monkeypatch)
    with pytest.raises(ValueError, match="Chunk 99999 not found"):
